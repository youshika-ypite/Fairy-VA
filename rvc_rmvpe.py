...
class E2E(nn.Module):
    def __init__(
        ...
        if n_gru:
            ...
            self.fc = nn.Sequential(
                nn.Linear(3 * N_MELS, N_CLASS), nn.Dropout(0.25), nn.Sigmoid() # type: ignore
            )
...
class RMVPE:
    def __init__(self, model_path, is_half, device=None):
        ...
        model = E2E(4, 1, (2, 2))
        ml = 'cuda:0' if torch.cuda.is_available() else 'cpu'
        ckpt = torch.load(model_path, map_location=ml, weights_only=True)
